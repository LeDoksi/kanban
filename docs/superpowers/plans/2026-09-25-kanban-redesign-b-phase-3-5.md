...
5. Блок `ungrouped` — перед списком, если `groups.length + pinnedEmpty.length > 0`, подзаголовок:
   ...
   ```
6. **KAN-125, выбор владельца по эскизу: «мягкая подложка».** Группа эпика (шапка + её карточки) лежит на едва заметной плашке, задачи без эпика — прямо на фоне. В `styles.css` новый токен `--color-group`: светлая `rgb(29 34 38 / 0.045)`, тёмная `rgb(255 255 255 / 0.035)` (в `@theme` и в тёмном `:root`). Обёртка группы в `Column.tsx`:
   ```tsx
   ...
   <div key={epic.id} className="rounded-[18px] bg-(--color-group) p-1.5 space-y-1.5">
     <EpicGroupHeader … />
     <div className="space-y-1.5">…карточки…</div>
   </div>
   ```
   Пустые эпики в Backlog — та же плашка с одной шапкой внутри.
